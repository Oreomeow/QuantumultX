...
if (url.indexOf(p2) != -1) {
    (obj.data.isVip = 1), (obj.data.isVip_forever = 1), (obj.data.day = '永久会员'), (body = JSON.stringify(obj));
}
if (url.indexOf(p3) != -1) {
    (obj.data.is_free = 1), (obj.data.buyStatus = 1), (body = JSON.stringify(obj));
}
if (url.indexOf(p4) != -1) {
    (obj.data.is_free = 1), (obj.data.free_time = 999), (obj.data.buyStatus = 1), (body = JSON.stringify(obj));
}
if (url.indexOf(p1) != -1) {
    (obj.data.is_free = 1), (obj.data.free_time = 999), (obj.data.buyStatus = 1), (body = JSON.stringify(obj));
}
